"""
This script contains relative or absolute paths for local data
(i.e. dataframes for pretraining/transferability, data paths,
 and results paths)
"""

# Path with datasets
PATH_DATASETS = ".../Datasets/FUNDUS/"

# Path with pretraining and transferability dataframes
PATH_DATAFRAME_PRETRAIN = "./local_data/dataframes/pretraining/"
PATH_DATAFRAME_TRANSFERABILITY = "./local_data/dataframes/transferability/"
PATH_TRANSFERABILITY_CLASSIFICATION_SUBFOLDER = "classification"
PATH_DATAFRAME_TRANSFERABILITY_CLASSIFICATION = (
    PATH_DATAFRAME_TRANSFERABILITY + f"{PATH_TRANSFERABILITY_CLASSIFICATION_SUBFOLDER}/"
)

PATH_TRANSFERABILITY_SEGMENTATION_SUBFOLDER = "segmentation"
PATH_DATAFRAME_TRANSFERABILITY_SEGMENTATION = (
    PATH_DATAFRAME_TRANSFERABILITY + f"{PATH_TRANSFERABILITY_SEGMENTATION_SUBFOLDER}/"
)

# Paths for results
PATH_RESULTS_PRETRAIN = "./local_data/results/pretraining/"
PATH_RESULTS_TRANSFERABILITY = "./local_data/results/transferability/"
