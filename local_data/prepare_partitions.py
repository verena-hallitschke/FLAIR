...
from local_data.constants import (
    ...
    PATH_TRANSFERABILITY_SEGMENTATION_SUBFOLDER,
)


DatasetPreparationConfig = namedtuple(
    "DatasetPreparationConfig",
    [
        "datasets_base_path",
        "pretrain_dataset_output_path",
        "transferability_dataset_output_path",
        "classification_transferability_subfolder",
        "segmentation_transferability_subfolder",
    ],
)


def adequate_01_eyepacs(dataset_preparation_config: DatasetPreparationConfig):
    ...
    for iPartition in partitions:
        ...
        dataframe = pd.read_csv(
            os.path.join(
                dataset_preparation_config.datasets_base_path,
                path_dataset,
                f"{iPartition}.csv",
            )
        )
        ...
        for iFile in range(dataframe.shape[0]):
            ...
            if os.path.isfile(
                os.path.join(dataset_preparation_config.datasets_base_path, image_path)
            ):
                data.append(
                    {
                        "image": image_path,
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "01_EYEPACS.csv"
        )
    )


def adequate_02_messidor(dataset_preparation_config: DatasetPreparationConfig):
    labels_dr = {
        0: "no diabetic retinopathy",
        1: "mild diabetic retinopathy",
        2: "moderate diabetic retinopathy",
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.transferability_dataset_output_path,
            dataset_preparation_config.classification_transferability_subfolder,
            "02_MESSIDOR.csv",
        )
    )


def adequate_03_idrid(dataset_preparation_config: DatasetPreparationConfig):
    ...
    labels_dme = {
        0: "no referable diabetic macular edema",
        1: "non clinically significant diabetic macular edema",
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "03_IDRID.csv"
        )
    )
    ...
    for iPartition in range(len(partitions)):
        for iCategory in range(len(categories_paths)):
            ...
            df_out.to_csv(
                os.path.join(
                    dataset_preparation_config.transferability_dataset_output_path,
                    dataset_preparation_config.segmentation_transferability_subfolder,
                    f"03_IDRID_{annotations_categories[iCategory]}_{partitions_names[iPartition]}.csv",
                )
            )


def adequate_04_rfmid(dataset_preparation_config: DatasetPreparationConfig):
    template_diseases = {
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "04_RFMid.csv"
        )
    )


def adequate_05_1000x39(dataset_preparation_config: DatasetPreparationConfig):
    categories_template = {
        "0.0.Normal": "normal",
        ...
        "3.RAO": "central retinal artery occlusion",
        "4.Rhegmatogenous RD": "retina detachment",
        "5.0.CSCR": "central serous retinopathy",
        "5.1.VKH disease": "Vogt-Koyanagi syndrome",
        "6.Maculopathy": "maculopathy",
        "7.ERM": "epiretinal membrane",
        "8.MH": "macular hole",
        "9.Pathological myopia": "pathologic myopia",
        ...
        "18.Vitreous particles": "vitreous haemorrhage",
        "19.Fundus neoplasm": "neoplasm",
        ...
        "23.Vessel tortuosity": "tortuous vessels",
        "24.Chorioretinal atrophy-coloboma": "colobomas",
        "25.Preretinal hemorrhage": "preretinal haemorrhage",
        "26.Fibrosis": "fibrosis",
        "27.Laser Spots": "laser scar",
        "28.Silicon oil in eye": "silicon oil",
    ...
    categories = os.listdir(
        os.path.join(dataset_preparation_config.datasets_base_path, path_dataset)
    )
    ...
    for iCategory in categories:
        images = os.listdir(
            os.path.join(
                dataset_preparation_config.datasets_base_path, path_dataset, iCategory
            )
        )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "05_1000x39.csv"
        )
    )
...
def adequate_06_DEN(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "06_DEN"
    data = []
    ...
    for iPartition in partitions:
        f = open(
            os.path.join(
                dataset_preparation_config.datasets_base_path, path_dataset, iPartition
            )
        )
        meta = json.load(f)
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "06_DEN.csv"
        )
    )


def adequate_07_lag(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "07_LAG"
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "07_LAG.csv"
        )
    )


def adequate_08_odir5k(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "08_ODIR-5K"
    dataframe = pd.read_csv(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, "full_df.csv"
        )
    )

    # Train and Incremental test division
    mask_cat = np.logical_and(
        dataframe["C"].values == 1,
        dataframe[["D", "G", "C", "A", "H", "M", "O"]].values.sum(-1) == 1,
    )
    mask_myo = np.logical_and(
        dataframe["M"].values == 1,
        dataframe[["D", "G", "C", "A", "H", "M", "O"]].values.sum(-1) == 1,
    )
    mask_normal = np.logical_and(
        dataframe["N"].values == 1,
    ...
    for iFile in range(dataframe_train.shape[0]):
        ...
        for iEye in ["Right", "Left"]:
            ...
            if "myop" not in description and "cataract" not in description:
                ...
                if os.path.isfile(
                    os.path.join(
                        dataset_preparation_config.datasets_base_path, image_path
                    )
                ):
                    data.append(
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "08_ODIR.csv"
        )
    )
...
def adequate_09_papila(dataset_preparation_config: DatasetPreparationConfig):
            ...
            data.append(
                {
                    ...
                    "atributes": [],
                    "categories": [
                        labels_glaucoma[dataframes[0]["Diagnosis"][iFile + 2]]
                    ],
                }
            )
            ...
            data.append(
                {
                    ...
                    "atributes": [],
                    "categories": [
                        labels_glaucoma[dataframes[1]["Diagnosis"][iFile + 2]]
                    ],
                }
            )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "09_PAPILA.csv"
        )
    )


def adequate_10_paraguay(dataset_preparation_config: DatasetPreparationConfig):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "10_PARAGUAY.csv"
        )
    )


def adequate_11_stare(dataset_preparation_config: DatasetPreparationConfig):
    ...
    for line in open(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, metadata
        )
    ):
        categories, atributes = [], []
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "11_STARE.csv"
        )
    )


def adequate_12_aria(dataset_preparation_config: DatasetPreparationConfig):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "12_ARIA.csv"
        )
    )


def adequate_13_fives(dataset_preparation_config: DatasetPreparationConfig):
    ...
    for iSubpath in images_subpath:
        files = os.listdir(
            os.path.join(
                dataset_preparation_config.datasets_base_path, path_dataset, iSubpath
            )
        )
        for iFile in files:
            if iFile != "Thumbs.db":
...
def adequate_14_agar300(dataset_preparation_config: DatasetPreparationConfig):
    ...
    files = os.listdir(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, images_subpath
        )
    )
    for iFile in files:
        data.append(
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "14_AGAR300.csv"
        )
    )


def adequate_15_aptos(dataset_preparation_config: DatasetPreparationConfig):
    ...
    dataframe = pd.read_csv(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, "train.csv"
        )
    )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "15_APTOS.csv"
        )
    )


def adequate_16_fundoct(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "16_FUND-OCT"
    data = []
    dict_macula = {
        "acute CSR": "acute central serous retinopathy",
        "chronic CSR": "chronic central serous retinopathy",
        "ci-DME": "cystoid macular edema",
        ...
        data.append(
            {
                ...
                "atributes": [],
                "categories": [
                    iFile.replace(dataset_preparation_config.datasets_base_path, "")
                    .split("/")[3]
                    .lower()
                    .replace("healthy", "normal")
                ],
            }
        )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "16_FUND-OCT.csv"
        )
    )
...
def adequate_17_diaretdb1(dataset_preparation_config: DatasetPreparationConfig):
    ...
    files = os.listdir(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, subpath_images
        )
    )
    data = []
    for iFile in files:
            ...
            for item in root.findall("./markinglist/marking/"):
                if item.tag == "markingtype":
                    categories.append(
                        item.text.lower()
                        .replace("_", " ")
                        .replace("irma", "intraretinal microvascular abnormalities")
                    )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "17_DiaRetDB1.csv"
        )
    )


def adequate_18_drions_db(dataset_preparation_config: DatasetPreparationConfig):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "18_DRIONS-DB.csv"
        )
    )


def adequate_19_drishtigs1(dataset_preparation_config: DatasetPreparationConfig):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path,
            "19_Drishti-GS1.csv",
        )
    )


def adequate_20_e_ophta(dataset_preparation_config: DatasetPreparationConfig):
    ...
    for iSub in subpath_images:
        ...
        for root, dirs, files in os.walk(
            os.path.join(
                dataset_preparation_config.datasets_base_path, path_dataset, iSub
            )
        ):
            for filename in files:
                if filename != "Thumbs.db":
                    ...
                    data.append(
                        {
                            "image": os.path.join(root, filename).replace(
                                dataset_preparation_config.datasets_base_path, ""
                            ),
                            "atributes": [],
                            "categories": [finding],
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "20_E-ophta.csv"
        )
    )


def adequate_21_g1020(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "21_G1020"
    image_subpath = "Images"
    labels = {0: "normal", 1: "glaucoma"}
    dataframe = pd.read_csv(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, "G1020.csv"
        )
    )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "21_G1020.csv"
        )
    )


def adequate_22_heimed(dataset_preparation_config: DatasetPreparationConfig):
...
def adequate_23_hrf(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "23_HRF"
    data = []
    ...
    files = os.listdir(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, image_subpath
        )
    )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "23_HRF.csv"
        )
    )


def adequate_24_origa(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "24_ORIGA"
    image_subpath = "Images"
    labels = {0: "no glaucoma", 1: "glaucoma"}
    dataframe = pd.read_csv(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, "OrigaList.csv"
        )
    )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "24_ORIGA.csv"
        )
    )


def adequate_25_refuge(dataset_preparation_config: DatasetPreparationConfig):
...
def adequate_26_roc(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "26_ROC"
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "26_ROC.csv"
        )
    )


def adequate_27_brset(dataset_preparation_config: DatasetPreparationConfig):
    path_dataset = "27_BRSET"
    image_subpath = "fundus_photos"
    dataframe = pd.read_csv(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, "labels.csv"
        )
    )
    ...
    for iFile in range(dataframe.shape[0]):
        ...
        # Noise
        if (
            dataframe["focus"].values[iFile] == 2
            or dataframe["iluminaton"].values[iFile] == 2
            or dataframe["image_field"].values[iFile] == 2
            or dataframe["image_field"].values[iFile] == 2
        ):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "27_BRSET.csv"
        )
    )


def adequate_28_OIA(dataset_preparation_config: DatasetPreparationConfig):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "28_OIA-DDR.csv"
        )
    )


def adequate_29_airogs(dataset_preparation_config: DatasetPreparationConfig):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "29_AIROGS.csv"
        )
    )


def adequate_30_sustech(dataset_preparation_config: DatasetPreparationConfig):
    ...
    for iFile in files:
        ...
        if np.argwhere(row_mask.values).__len__() > 0:
            ...
            if os.path.isfile(
                ...
                data.append(
                    {"image": image_path, "atributes": [], "categories": findings}
                )
...
def adequate_31_jichi(dataset_preparation_config: DatasetPreparationConfig):
    ...
    labels_dr = {
        ...
        "pdr": [
            ...
            "fibrovascular proliferativemembrane",
    ...
    dataframe = pd.read_csv(
        os.path.join(
            dataset_preparation_config.datasets_base_path, path_dataset, "list.csv"
        )
    )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "31_JICHI.csv"
        )
    )


def adequate_32_chaksu(dataset_preparation_config: DatasetPreparationConfig):
        ...
        for iScanner in scanners:
            ...
            for iFile in files:
                ...
                if os.path.isfile(
                    ...
                    finding = labels[
                        dataframe[dataframe["Images"] == iFile][ky].values[0]
                    ]
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "32_CHAKSU.csv"
        )
    )


def adequate_33_dr(dataset_preparation_config: DatasetPreparationConfig):
    ...
    transfer = {
        ...
        "Deep Hemorrhages": "deep haemorrhages",
        "Drusen": "drusens",
        "Hard Exudates": "hard exudates",
        "Normal Images": "normal",
        "Red Lesions": "red small dots",
        "Superficial Hemorrhages": "superficial haemorrhages",
    }
    data = []
    for iPath in subpaths:
        files = os.listdir(
            os.path.join(
                dataset_preparation_config.datasets_base_path, path_dataset, iPath, "/"
            )
        )
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "33_DR1-2.csv"
        )
    )


def adequate_34_cataract(dataset_preparation_config: DatasetPreparationConfig):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "34_Cataract.csv"
        )
    )


def adequate_35_scardat(dataset_preparation_config: DatasetPreparationConfig):
    ...
    df_out.to_csv(
        os.path.join(
            dataset_preparation_config.pretrain_dataset_output_path, "35_ScarDat.csv"
        )
    )


def adequate_36_acrima(dataset_preparation_config: DatasetPreparationConfig):
...
def adequate_37_DeepDRiD(dataset_preparation_config: DatasetPreparationConfig):
    ...
    for i in range(0, len(paritions)):
        ...
        for ii in range(dataframe.shape[0]):
            iFile = (
                dataframe["image_path"][ii]
                .replace("\\", "/")
                .replace(paritions_path[i][:-1], "Images")
            )
            dr = np.nanmean(
                dataframe[["left_eye_DR_Level", "right_eye_DR_Level"]].values[ii, :]
            )
...
def adequate_CGI_HRDC(dataset_preparation_config: DatasetPreparationConfig):
    # TASK 1
...
def prepare_pretraining_datasets(
    ...
    segmentation_transferability_subfolder: str = PATH_TRANSFERABILITY_SEGMENTATION_SUBFOLDER,
):
    ...
    if not os.path.exists(
        os.path.join(
            transferability_dataset_output_path, segmentation_transferability_subfolder
        )
    ):
        os.mkdir(
            os.path.join(
    ...
    dataset_preparation_config = DatasetPreparationConfig(
        ...
        classification_transferability_subfolder=classification_transferability_subfolder,
        segmentation_transferability_subfolder=segmentation_transferability_subfolder,
    )
